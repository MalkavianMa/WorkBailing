...
{
    /// <summary>
    /// 济南居民门诊统筹 Source:南部山区 from:历下区
    /// </summary>
    public class JNDWInterfaceModel_JMTC : IPayCompanyInterface
    {
        //医保个人信息 
        ...
        //    LogManager.Debug("医保统筹支付额：" + outSettleMain.MedAmountTotal);
        ...
        public void SaveInSettleMain()
        {
            ...
            inSettleMain.OperatorId = PayAPIConfig.Operator.UserSysId;
        ...
        public NetworkPatInfo NetworkReadCard()
        {
            ...
            //string id = outReimPara.PatInfo.IDNo;
            if (frmCard.iscard == "1")
            {
                CARD_Y_N = "1";
        ...
        {
            //outReimPara = para;
            LogManager.Debug("接口调用类居民统筹服务>>>门诊登记");
          //  NetworkReadCard();//371481199110050910
        ...
        private NetworkPatInfo NetworkReadCard(OutPayParameter para)
        {
            InterfaceInit();
            NetworkPatInfo networkPatInfo = new NetworkPatInfo();
        ...
        public void OutNetworkPreSettle(OutPayParameter para)
        {
            //InterfaceInit();
            outReimPara = para;
            //handelModel.InitMZMG(outReimPara.RegInfo.CantonCode, "6", outReimPara.RegInfo.MemberNo, outReimPara.PatInfo.PatName,
        ...
        public void OutNetworkSettle(OutPayParameter para)
        {
            try
            {
                ...
                //handelModel.InitJMMZ(outReimPara.RegInfo.CantonCode, "4", outReimPara.RegInfo.MemberNo, outReimPara.PatInfo.PatName,
                //                    dicPatInfo["xb"], outReimPara.CommPara.OutNetworkSettleId.ToString(), DateTime.Now.ToString("yyyy-MM-dd"),//"001", 
                //                    PayAPIConfig.Operator.UserSysId, strDiagnosCode, syzhlx, ylzbh);

                LogManager.Debug("门诊结算初始化统筹号：" + outReimPara.RegInfo.CantonCode);
                ...
                //....outReimPara
                //费用明细上传
                //不取 string ysbmdw = outReimPara.Details[0].DocCode;
        ...
        public void CancelOutNetworkSettle(OutPayParameter para)
        {
            ...
            InterfaceInit();
            LogManager.Debug("初始化成功，开始 撤销门诊结算》》");
            outReimPara = para;
            ...
            while (true)
            {
                try
                {
                    ...
                    //{//
                    //    ylzbh = regInfoCardNo;
                    //    if (string.IsNullOrEmpty(ylzbh))
                    //    {
                    //        throw new Exception("接口 提示ylzbh为空 无法撤销结算");
                    //    }
                    //    // ylzbh = outReimPara.RegInfo.CardNo;
                    //    syzhlx = "0";
                    //}
                    ////正式需修改   结算时可用扩展值存
                    ////  string grion = netPatInfo.MedicalNo;
                    ////  outReimPara.RegInfo.CantonCode = "370191";
                    ////outReimPara.RegInfo.MemberNo = "37011220111123034x";
                    ...
                    ////是否sQL
                    //handelModel.InitJMMZ(regInfoCantonCode, "6", regInfoMemberNo, outReimPara.PatInfo.PatName,
                    //           dicPatInfo["xb"], outReimPara.CommPara.OutNetworkSettleId.ToString(), DateTime.Now.ToString("yyyy-MM-dd"),//"001", 
                    //           PayAPIConfig.Operator.UserSysId, strDiagnosCode, syzhlx, ylzbh); 
                    #endregion
                    LogManager.Debug("开始撤销门诊结算，流水号：" + settleNo);
        ...
        public void InNetWorkReadCard(InPayParameter para)
        {
            ...
            //判断有卡无卡
            // frmCARD frmCard = new frmCARD();
            string quickIDnumber = para.PatInfo.IDNo;
            frmCARD frmCard = new frmCARD(quickIDnumber);
            frmCard.ShowDialog();
            if (frmCard.iscard == "1")
            {
        ...
        {
            throw new Exception("该类别为门诊类别,无法使用住院业务");
            InterfaceInit();
            inReimPara = para;
        ...
        public void InNetworkPreSettle(InPayParameter para)
        {
            ...
            if (inReimPara.SettleInfo != null && inReimPara.SettleInfo.MedAmountTotal != 0)
            {
                ...
                payType.PayTypeId = 4;
                payType.PayTypeName = "医保";
                payType.PayAmount = Convert.ToDecimal(inReimPara.SettleInfo.MedAmountTotal) - inReimPara.SettleInfo.MedAmountZhzf;
                inReimPara.PayTypeList.Add(payType);
                ...
                payType1.PayAmount = Convert.ToDecimal(inReimPara.SettleInfo.MedAmountZhzf);
                inReimPara.PayTypeList.Add(payType1);

                //   LogManager.Debug("监测到接口结算表存在结算数据,跳过。医保农合支付总额:" + inReimPara.SettleInfo.MedAmountTotal + "住院ID" + inReimPara.RegInfo.PatInHosId + "姓名" + inReimPara.RegInfo.NetPatName);
            ...
            if (inReimPara.Details.Count > 0)
            {
                ...
                //if (ybkhavecard=="1")
                {
                    ...
                    payType.PayTypeId = 4;
                    payType.PayTypeName = "医保";
                    payType.PayAmount = Convert.ToDecimal(inSettleMain.MedAmountTotal) - inReimPara.SettleInfo.MedAmountZhzf;
                    inReimPara.PayTypeList.Add(payType);
        ...
        public void InNetworkSettle(InPayParameter para)
        {
            ...
            try
            {
                ...
                if (inReimPara.SettleInfo != null && inReimPara.SettleInfo.SettleNo != "")//inReimPara.SettleInfo.MedAmountTotal     inReimPara.SettleInfo.SettleNo
                {
                    ...
                    payType1.PayAmount = inReimPara.SettleInfo.MedAmountZhzf;
                    inReimPara.PayTypeList.Add(payType1);
                    LogManager.Debug("出院结算检测到结算表里有值跳过医保厂商结算 易宝支付：" + payType.PayAmount + " 医保卡支付:" + payType1.PayAmount + "");
