...
namespace DW_YBBX
{
    class DW_Handle
    {
        ...
        {
            return seiproxy.ExeFuncReStr("getvarstring", new object[] { strPara });
        }
        /// <summary>
        /// 获取返回数值
        /// </summary>
        ...
        {
            return seiproxy.ExeFuncReDec("getvardec", new object[] { strPara });
        }
        ...
        {
            seiproxy.ExeFuncReStr("resetvar", null);
        }
        ...
        {
            seiproxy = new RefCOM("sei3");
            string hosNo = MainForm.yybm;
            string usercode = MainForm.UserCode_DW;
            string password = MainForm.Password_DW;
            ClearInPara();
            SetInParaString("yybm", hosNo);
            SetInParaString("gzrybh", usercode);
            SetInParaString("pwd", password);
            decimal iRe = ExecService("init");
            if (iRe != 0)
            {
        ...
        public void Down_Yyxm(string sbjgbh, string filename, long filetype, long has_head)
        {
            ...
            SetInParaDec("filetype", filetype);
            SetInParaDec("has_head", has_head);
            decimal iRe = ExecService("down_yyxm");
            if (iRe != 0)
            {
        ...
        public void Down_Ml(string sbjgbh, string filename, string filename2, long filetype, long has_head)
        {
            ...
            SetInParaString("sbjgbh", sbjgbh);
            SetInParaString("filename", filename);
            SetInParaString("filename2", filename2);
            SetInParaDec("filetype", filetype);
            SetInParaDec("has_head", has_head);
            decimal iRe = ExecService("down_ml_with_xz");
            if (iRe != 0)
            {
                throw new Exception("下载医院项目出错：" + seiproxy.ExeFuncReStr("get_errtext", null));
            }
        ...
        public void add_yyxm_info_all(DataTable dt)
        {
            ...
            {
                SetInParaString("ypbz", row["ypbz"].ToString().Replace(" ", ""));
                SetInParaString("mllb", row["mllb"].ToString());
                SetInParaString("yyxmbm", row["yyxmbm"].ToString().Replace(" ", ""));
                SetInParaString("yyxmmc", row["yyxmmc"].ToString());
                SetInParaString("zyjsxmbh", "999");
                SetInParaString("mzjsxmbh", "999");
                SetInParaString("zxgg", row["zxgg"].ToString());
                SetInParaString("syz", row["syz"].ToString());
                SetInParaString("jj", row["jj"].ToString());
                SetInParaString("scqy", row["scqy"].ToString());
                SetInParaString("spm", row["spm"].ToString());
                SetInParaString("dw", row["dw"].ToString());
                SetInParaString("gmpbz", row["gmpbz"].ToString());
                SetInParaString("cfybz", row["cfybz"].ToString());
                SetInParaString("zdgg", row["zdgg"].ToString());
                SetInParaString("jxmc", row["jxmc"].ToString());
                SetInParaDec("dj", Convert.ToDecimal(row["dj"].ToString()));
                SetInParaDec("bhsl", Convert.ToDecimal(row["bhsl"].ToString()));
                SetInParaDec("kcl", Convert.ToDecimal("0"));

                iRe = ExecService("add_yyxm_info_all");
                if (iRe != 0)
                {
                    errNum++;
